...
public class MainVerticle extends AbstractVerticle {
  ...
  private static final String SQL_CREATE_PAGES_TABLE = "\n" + 
      "\n" + 
      "create table  if not exists `javabase`.`Pages` (\n" + 
  ...
  private Future<Void> prepareDatabase() {
    System.out.println("chuan bi data base");
    Future<Void> future = Future.future();

    JsonObject mySQLClientConfig = new JsonObject().put("host", "localhost")
                                                .put("port", 3306)   // <2>
                                                .put("database", "javabase")   // <3>
                                                 .put("username", "root") 
                                                .put("password", "soldier001")
                                                .put("max_pool_size", 30); 
    /*dbClient = JDBCClient.createShared(vertx, new JsonObject()  // <1>
      .put("url", "jdbc:Mysql://localhost:3306/javabase")   // <2>
      .put("driver_class", "com.mysql.jdbc.Driver")   // <3>
       .put("username", "root") 
      .put("password", "soldier001")
      .put("max_pool_size", 30));   // <4>*/
     dbClient = MySQLClient.createNonShared(vertx, mySQLClientConfig);
  ...
  public void start(Future<Void> startFuture) throws Exception {
    System.out.println("day la start server");
